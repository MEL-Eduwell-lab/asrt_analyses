...
execute:
  ...
  warning: false
  error: true
  working-directory: "/Users/coum/Documents/GitHub/asrt_analyses"
editor: visual
python:
...
# Dynamically add the parent directory (root) to the front of the search path.
# This works regardless of whether Quarto thinks it's in 05_gam or the root.
sys.path.insert(0, os.path.abspath('..'))
sys.path.insert(0, os.path.abspath('.'))
sys.path.insert(0, "/Users/coum/Documents/GitHub/asrt_analyses")
...
# Now these should import perfectly
from base import ensure_dir, ensured, get_all_high_low, get_sequence, fisher_z_and_ttest
from config import *
```
...
# fname = f'rsa_sensors_tr_step{step}.csv'
...
df = pd.DataFrame(rows)
# fname = f'rsa_sensors_tr_corr_step{step}.csv'
# df.to_csv(data_path / fname, index=False, sep=",")
```
...
df = pd.DataFrame(rows)
# fname = f'rsa_sensors_tr_no_prac_corr_step{step}.csv'
# df.to_csv(data_path / fname, index=False, sep=",")
```
...
```{r}
df.tmp <- read_csv(file.path(as.character(py$figures_dir), "behav/learning_indices_blocks.csv"))
...
df.tmp %>%
  ...
  write_csv(paste0("./gam/data/behav_blocks_sensors_step", py$step, ".csv"))

df.sensors <- bind_rows(
  read_csv(paste0("./05_gam/gam/data/rsa_sensors_tr_step", py$step, ".csv")) %>% mutate(metric = "RS", .before = 1),
  read_csv(paste0("./05_gam/gam/data/rsa_sensors_tr_corr_step", py$step, ".csv")) %>% mutate(metric = "RS CORR", .before = 1),
) %>%
...
# emm_segments
write.csv(emm_segments, paste0("./gam/data/segments_rs_tr_sensors_step", py$step, ".csv"), row.names=FALSE)
```
...
segments = pd.read_csv(data_path / f"segments_rs_tr_sensors_step{step}.csv")
...
df = pd.DataFrame(rows)
fname = f'rsa_sensors_br_step{step}.csv'
df.to_csv(data_path / fname, index=False, sep=",")
```
...
df = pd.DataFrame(rows)
fname = f'rsa_source_tr_step{step}.csv'
df.to_csv(data_path / fname, index=False, sep=",")
```
...
df = pd.DataFrame(rows)
fname = f'rsa_source_tr_corr_step{step}.csv'
df.to_csv(data_path / fname, index=False, sep=",")
```
...
df = pd.DataFrame(rows)
fname = f'rsa_source_tr_no_prac_corr_step{step}.csv'
df.to_csv(data_path / fname, index=False, sep=",")
```
...
```{r}
df.source <- bind_rows(
  read_csv(paste0("./gam/data/rsa_source_tr_step", py$step, ".csv")) %>% mutate(metric = "RS", .before = 1),
  read_csv(paste0("./gam/data/rsa_source_tr_corr_step", py$step, ".csv")) %>% mutate(metric = "RS CORR", .before = 1)
) %>%
  mutate(across(c(network, subject), as.factor)) %>%
  mutate(across(c(time, value), as.numeric))

df.source %>% mutate(NA.values = is.na(value)) %>% count(network, metric, time, NA.values)

n.time.source <- df.source$time %>% unique %>% length()
```

## Fit models

```{r}
models.source.rs <- df.source %>%
...
model.source.all <- df.source %>%
  mutate(value_std = scale(value, center = FALSE, scale = TRUE)) %>%
  mutate(metric = as.factor(metric),
         network = as.factor(network),
         network_metric = interaction(network, metric)) %>%
  gam(data = .,
      value_std ~ s(time, k = 10) +
        s(network_metric, time, bs = "sz") +
        s(subject, bs = "re") + # random intercept
        s(subject, time, bs = "re"), # random linear slope
      method = "REML")
```
...
```{r}
# Apply emmeans
tmp.source <- models.source.fs %>%
  group_by(network, metric) %>%
  mutate(
    time_seq = map2(min_time, max_time, ~ seq(.x, .y, length.out = n.time.source)),
    emm = map2(gam_model, time_seq, ~ as_tibble(emmeans(.x, ~ time, at = list(time = .y))))
  )

# Store emm + SE
emm_meanse.source <- tmp.source %>%
  select(network, metric, emm) %>%
  unnest(emm)

# Extract segments of significance
emm_segments.source <- tmp.source %>%
  mutate(
    segs = map(emm, ~ {
      df <- .x %>% mutate(signif = (lower.CL > 0) | (upper.CL < 0))
      r <- rle(df$signif)
  ...
  select(network, metric, segs) %>%
  unnest(segs)

write.csv(emm_segments.source, paste0("./gam/data/segments_rs_tr_source_step", py$step, ".csv"), row.names = FALSE)
```

## Export average RS effect per block and ROIs

```{python}
# extract network-wise index from source GAMM segments
segments = pd.read_csv(data_path / f"segments_rs_tr_source_step{step}.csv")
segments_rs = segments[segments["metric"] == "RS"].copy()
cluster_idx = 0 if data_type.endswith("new") else 1

# save table
rows = list()
for i, network in enumerate(networks):
    network_label = network_names[i]
    seg_network = segments_rs[segments_rs["network"] == network_label].reset_index(drop=True)
    if seg_network.empty:
        continue
    if cluster_idx >= len(seg_network):
        cluster_idx_use = 0
    else:
        cluster_idx_use = cluster_idx
    start = seg_network.iloc[cluster_idx_use]["start"]
    end = seg_network.iloc[cluster_idx_use]["end"]
    idx = np.arange(int(start), int(end) + 1)
...
df = pd.DataFrame(rows)
fname = f'rsa_source_br_step{step}.csv'
df.to_csv(data_path / fname, index=False, sep=",")
```
