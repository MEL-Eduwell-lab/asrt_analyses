...
if not use_decode:
    ...
    seg_df = pd.read_csv(FIGURES_DIR / "TM" / "em_segments_pa_tr_source_subreg_merged.csv")
    ...
    sig_df = pd.read_csv(FIGURES_DIR / "TM" / "smooth_pa_tr_source_subreg_merged.csv")
...
for network in networks:
    ...
    # Get label names matching the directory names produced by timeg_net_subreg_merged.py:
...
if saving:
    pd.DataFrame(all_rows).to_csv(
        figures_dir / "timeg_net_subreg_merged_source_data.csv", index=False
    )
