...
# export fiducials to txt file
from base import ensured
fid_res_path = ensured(RESULTS_DIR / "fiducials")
for subject in subjects:
    fid = mne.coreg.get_mni_fiducials(subject, subjects_dir)
    print(f"Writing fiducials for {subject}...")
    fid_fname = f"{subject}-fid.fif"
    mne.io.write_fiducials(fid_res_path / fid_fname, fid, coord_frame='mri', overwrite=True, verbose=verbose)

# check
fid = mne.io.read_fiducials(fid_res_path / fid_fname)
print(fid)
