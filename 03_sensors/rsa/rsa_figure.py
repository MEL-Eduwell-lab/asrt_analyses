...
for start, end in contiguous_regions(sig):
    axd['B'].plot(times[start:end], diff_lh.mean(0)[start:end], alpha=1, zorder=10, color=c2)
axd['B'].fill_between(times, diff_lh.mean(0) - sem, diff_lh.mean(0) + sem, where=sig, alpha=0.4, zorder=5, facecolor=c2)
axd['B'].fill_between(times, diff_lh.mean(0) - sem, 0, where=sig, alpha=0.3, zorder=5, facecolor=c2)
axd['B'].text(np.mean(times[sig]), -0.1, '***', fontsize=25, ha='center', va='center', color=c2, weight='bold')
axd['B'].set_ylabel('Similarity index', fontsize=11)
...
for start, end in contiguous_regions(sig):
    axd['C'].plot(times[start:end], np.nanmean(all_rhos, axis=0)[start:end], alpha=1, zorder=10, color=c6)
    cluster_center = np.mean(times[start:end])
    axd['C'].text(cluster_center, -0.07, '*', fontsize=25, ha='center', va='center', color=c6, weight='bold')
axd['C'].fill_between(times, np.nanmean(all_rhos, axis=0) - sem, np.nanmean(all_rhos, axis=0) + sem, where=sig, alpha=0.4, zorder=10, facecolor=c6)
axd['C'].fill_between(times, np.nanmean(all_rhos, axis=0) - sem, 0, where=sig, alpha=0.3, zorder=5, facecolor=c6)
axd['C'].set_ylabel("Spearman's rho", fontsize=11)
axd['C'].set_xlabel('Time (s)', fontsize=11)
axd['C'].set_title('Similarity index and learning correlation time course', fontsize=13)

### D ### Learning index fit
